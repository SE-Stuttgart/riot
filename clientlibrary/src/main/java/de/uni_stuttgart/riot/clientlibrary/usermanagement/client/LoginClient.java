...
public class LoginClient {
	...
	public LoginClient(String serverUrl, String deviceName,
			TokenManager tokenManager, DefaultHttpClient client) {
	...
	public User login(String username, String password)
			throws RequestException, ClientProtocolException, IOException {
		return this.internalLogin(LOGIN_PATH, new LoginRequest(username,
				password));
	}
	 ...
	 * @throws IOException .
	 */
	public void logout() throws RequestException, JsonGenerationException,
	...
	User internalLogin(String path, Object entity) throws RequestException {
		try {
			...
			StringEntity jsonEntity = new StringEntity(
					LoginClient.this.jsonMapper.writeValueAsString(entity));
			...
			if (status >= BAD_REQUEST) {
				if (mediaType.equals(APPLICATION_JSON)) {
					RequestExceptionWrapper error = LoginClient.this.jsonMapper
							.readValue(r.getEntity().getContent(),
									RequestExceptionWrapper.class);
					error.throwIT();
				} else {
					String result = EntityUtils.toString(r.getEntity());
					RequestExceptionWrapper error = new RequestExceptionWrapper(
							status, result);
					error.throwIT();
				}
			} else {
				AuthenticationResponse response = LoginClient.this.jsonMapper
						.readValue(r.getEntity().getContent(),
	...
	HttpResponse put(final String target, final Object entity)
			throws RequestException {
		try {
			return this.doRequest(new InternalRequest() {
				public HttpResponse doRequest() throws JsonGenerationException,
						JsonMappingException, UnsupportedEncodingException,
						IOException {
					HttpPut put = new HttpPut(target);
					put.setHeader(ACCESS_TOKEN, tokenManager.getAccessToken());
					StringEntity jsonEntity = new StringEntity(
							LoginClient.this.jsonMapper
	...
	HttpResponse post(final String target, final Object entity)
			throws RequestException {
		try {
			return this.doRequest(new InternalRequest() {
				public HttpResponse doRequest() throws JsonGenerationException,
		...
		try {
			...
			if (response.getEntity() != null) {
				if (response.getEntity().getContentType() != null) {
					mediatype = response.getEntity().getContentType()
							.getValue();
				}
			}
