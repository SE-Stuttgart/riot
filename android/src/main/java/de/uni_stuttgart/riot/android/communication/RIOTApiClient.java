...
/**
 * Client for communicating with the RIOT API. It is not allowed to use this
...
public class RIOTApiClient {
	...
	private static final String API_URL = "https://belgrad.informatik.uni-stuttgart.de:8181"; // TODO
																								// url,
																								// 10.0.2.2
																								// is
																								// localhost
	...
	/**
	 * Gets the single instance of RIOTApiClient. It is <b>NOT</b> allowed to
	 * use the RIOTApiClient in the main thread!
	 *
	...
	/**
	 * Initializes the API Client. Has to be called before first usage or a
	 ...
	 *            the device name
	 */
	public void init(HomeScreen context, String deviceName) {
		throwExceptionIfOnMainThread();

		try {
			loginClient = new LoginClient(API_URL, deviceName,
					new TokenManager(context), new MyHttpClient(context));
		} catch (IOException e) {
	...
	private void checkIfInit() {
		if (!init) {
			throw new RuntimeException(
					"RiotApiClient has to be initialized before first usage. Call init() first!");
		}
	}
